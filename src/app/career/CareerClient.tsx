...
import type { Job, SiteConfig } from '@/lib/cms';
import {
  MapPin,
  Clock,
  Briefcase,
...
const FALLBACK_EMPLOYEE_STORIES = [
  {
...
const FALLBACK_WHY_WORK_WITH_US = [
  {
    icon: 'TrendingUp',
...
const FALLBACK_OPENINGS: Job[] = [
  {
    id: 0,
    title: 'Senior Marketing Manager',
    slug: 'senior-marketing-manager',
...
export function CareerClient({ jobs, siteConfig }: CareerClientProps) {
  const openings = jobs.length > 0 ? jobs : FALLBACK_OPENINGS;
  const whyWorkWithUs =
    siteConfig?.whyWorkWithUs && siteConfig.whyWorkWithUs.length > 0
      ? siteConfig.whyWorkWithUs
      : FALLBACK_WHY_WORK_WITH_US;
  const employeeStories =
    siteConfig?.employeeStories && siteConfig.employeeStories.length > 0
      ? siteConfig.employeeStories
      : FALLBACK_EMPLOYEE_STORIES;
