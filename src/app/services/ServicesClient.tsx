...
function SectorSection({ sector, index }: { sector: CmsSector; index: number }) {
  const isEven = index % 2 === 0;
  const Icon = resolveIcon(sector.icon);

  // Split the long description into a bullet list of services
...
export function ServicesClient({ sectors }: { sectors: CmsSector[] }) {
  return (
    <>
      <PageHero
